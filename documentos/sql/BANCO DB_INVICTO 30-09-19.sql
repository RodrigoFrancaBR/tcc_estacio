...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_UNIDADE` (
  ...
  `situacao` TINYINT() NOT NULL DEFAULT 1 COMMENT 'USADO PARA EXCLUSÃO LÓGICA - 0 desativado 1 ativado',
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_ALUNO` (
  ...
  INDEX `FK_TB_PESSOA_TB_TURMA_IDX` (`turma_id` ASC),
  UNIQUE INDEX `cpf_UNIQUE` (`cpf` ASC),
  UNIQUE INDEX `rg_UNIQUE` (`rg` ASC),
  CONSTRAINT `FK_TB_PESSOA_TB_TURMA_TB_TURMA`
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_CONTRATO` (
  ...
  `situacao` TINYINT() NOT NULL DEFAULT 1 COMMENT 'USADO PARA EXCLUSÃO LÓGICA - 0 desativado 1 ativado 2 EM ACORDO 3 ENCERRADO 4 SIMULAÇÃO' ,
  -- `situacao_matricula` VARCHAR(45) NOT NULL COMMENT 'PROCESSO DE MATRÍCULA, situacao/MATRICULADO, CANCELADO, ACORDO, ENCERRADO',
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_FUNCIONARIO` (
  ...
  `matricula` VARCHAR(45) NOT NULL,
  `cargo` VARCHAR(45) NOT NULL COMMENT '0= não é professor, 1= professor',
  `nome` VARCHAR(45) NOT NULL,
  `cpf` VARCHAR(45) NOT NULL,
  `rg` VARCHAR(45) NOT NULL,
  `celular` VARCHAR(45) NOT NULL,
  `email` VARCHAR(45) NOT NULL,
  `situacao` VARCHAR(7) NOT NULL COMMENT 'USADO PARA EXCLUSÃO LÓGICA situacao/Insituacao',
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_CATEGORIA` (
  ...
  `nome` VARCHAR(45) NOT NULL,
  `tipo_categoria` VARCHAR(45) NOT NULL COMMENT '0 = indefinida, 1 = FUNCIONARIO, 2 = ESSENCIAL, 3 = ESCRITORIO, 4 = TERCEIROS',
  `situacao` VARCHAR(7) NOT NULL COMMENT 'USADO PARA EXCLUSÃO LÓGICA situacao/Insituacao',
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_DESPESA` (
  ...
  `data_vencimento` DATE NOT NULL COMMENT 'VENCIMENTO PREVISTO',
  `via_recebido` VARCHAR(45) NOT NULL COMMENT '??',
  `situacao` VARCHAR(7) NOT NULL COMMENT 'USADO PARA EXCLUSÃO LÓGICA \n0 = desativado, 1 = situacao, ',
...
CREATE TABLE IF NOT EXISTS `DB_INVICTO`.`TB_LANCAMENTO_DESPESA` (
  ...
  `situacao_lancamento` VARCHAR(45) NOT NULL,
  `data_emissao` VARCHAR(45) NULL,
  PRIMARY KEY (`id_lancamento_despesa`),
  INDEX `FK_LANCAMENTO_DESPESA_DESPESA_IDX` (`despesa_id` ASC),
